...
def patient_form() -> dict:
    with st.form("patient_form"):
        ...
        with c1:
            sample_id=st.text_input("Sample ID","CASE-EXPERT-001"); age=st.number_input("Age",0,120,24)
        with c2:
            sex=st.selectbox("Sex",["Female","Male","Other/Not specified"]); pregnant=st.checkbox("Pregnant / antenatal screening")
        with c3:
            transfusion_recent=st.checkbox("Recent transfusion"); family_history=st.checkbox("Family history / partner carrier known")
        with c4:
            smear_target_cells=st.checkbox("Target cells on smear"); hbh_inclusion=st.selectbox("HbH inclusion body",["Negative","Positive"])
        section("CBC, iron, and hemoglobin analysis")
        ...
        with b1: rdw=st.number_input("RDW (%)",5.0,35.0,14.2,.1)
        with b2: retic=st.number_input("Reticulocyte (%)",0.0,30.0,1.2,.1)
        with b3: ferritin=st.number_input("Ferritin (ng/mL)",0.0,2000.0,85.0,1.0)
        with b4: hba=st.number_input("HbA (%)",0.0,100.0,94.0,.1)
        with b5: hba2=st.number_input("HbA2 (%)",0.0,100.0,5.1,.1)
        with b6: hbf=st.number_input("HbF (%)",0.0,100.0,1.2,.1)
        c1,c2,c3=st.columns(3)
        with c1: hbe=st.number_input("HbE / variant window (%)",0.0,100.0,0.0,.1)
        with c2: oft=st.selectbox("Osmotic fragility test",["Negative","Positive"],index=1)
        with c3: dcip=st.selectbox("DCIP for HbE/unstable Hb",["Negative","Positive"])
        st.form_submit_button("Run expert consult", type="primary")
    return {"sample_id":sample_id,"age":age,"sex":sex,"hb_g_dl":hb,"rbc_10e12_l":rbc,"hct_percent":hct,"mcv_fl":mcv,"mch_pg":mch,"mchc_g_dl":mchc,"rdw_percent":rdw,"retic_percent":retic,"ferritin_ng_ml":ferritin,"hba_percent":hba,"hba2_percent":hba2,"hbf_percent":hbf,"hbe_percent":hbe,"oft":oft,"dcip":dcip,"hbh_inclusion":hbh_inclusion,"transfusion_recent":transfusion_recent,"pregnant":pregnant,"smear_target_cells":smear_target_cells,"family_history":family_history}

if mode == "Single patient consult":
    row=patient_form(); result=analyze_screening(row, thresholds)
    section("Consult summary")
    k1,k2,k3,k4=st.columns(4); level="danger" if result.consult_risk=="Critical review" else "high" if result.consult_risk=="High" else "moderate" if result.consult_risk=="Moderate" else "low"
    with k1: metric_card("Risk class", result.consult_risk, result.top_pattern, level)
    with k2: metric_card("Consult score", f"{result.consult_score:.1f}", "Highest phenotype score / 100", level)
    with k3: metric_card("Mentzer index", str(result.derived.get("mentzer_index","NA")), "<13 supports thal-like microcytosis", "info")
    with k4: metric_card("Reflex priority", "Molecular" if result.consult_score>=45 else "Review", "Based on pattern strength and caveats", "high" if result.consult_score>=45 else "low")
    ...
    with t1:
        a,b=st.columns([.85,1.15])
        with a: st.plotly_chart(risk_gauge(result.consult_score), use_container_width=True)
        with b: st.plotly_chart(score_radar(scores), use_container_width=True)
        c,d=st.columns(2)
        with c: st.plotly_chart(hb_fraction_donut(row["hba_percent"],row["hba2_percent"],row["hbf_percent"],row["hbe_percent"]), use_container_width=True)
        with d: st.plotly_chart(cbc_reference_bars(row), use_container_width=True)
    with t2:
        a,b=st.columns(2)
        with a: st.plotly_chart(hplc_chromatogram(row["hba_percent"],row["hba2_percent"],row["hbf_percent"],row["hbe_percent"]), use_container_width=True)
        with b: st.plotly_chart(mcv_hba2_quadrant(row["mcv_fl"],row["hba2_percent"]), use_container_width=True)
        st.plotly_chart(diagnostic_waterfall(result), use_container_width=True)
    with t3: st.plotly_chart(reflex_sankey(result), use_container_width=True)
    section("Interpretive evidence")
    col1,col2,col3=st.columns(3)
    with col1: clinical_box("<b>Evidence</b><br>"+"<br>".join([f"• {x}" for x in result.evidence]), "success" if result.consult_score<45 else "warn")
    with col2: clinical_box("<b>Recommendations</b><br>"+"<br>".join([f"• {x}" for x in result.recommendations]), "warn")
    with col3: clinical_box("<b>Caveats</b><br>"+("<br>".join([f"• {x}" for x in result.caveats]) if result.caveats else "• No major caveat generated."), "danger" if result.caveats else "success")
    ...
    with k3: metric_card("Median score", f"{results['consult_score'].median():.1f}", "Population consult score", "moderate")
