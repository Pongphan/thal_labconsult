...
def diagnostic_waterfall(result) -> go.Figure:
    measures=["relative"]*5+["total"]; labels=["Microcytosis","Hypochromia","HbA2/HbE","RBC pattern","Iron modifier","Consult score"]
    vals=[15 if result.microcytosis else 0,10 if result.hypochromia else 0,max(0,min(40,result.beta_trait_score*.45+result.hbe_score*.35)),10 if result.derived.get("mentzer_index",99)<13 else 0,-12 if result.iron_deficiency_score>=45 and result.beta_trait_score>=30 else 0,result.consult_score]
...
def batch_mcv_hba2_scatter(df: pd.DataFrame) -> go.Figure:
    fig=px.scatter(df,x="mcv_fl",y="hba2_percent",color="top_pattern",size="consult_score",hover_name="sample_id",hover_data=["consult_risk","ferritin_ng_ml","mentzer_index"],labels={"mcv_fl":"MCV (fL)","hba2_percent":"HbA2 (%)"})
