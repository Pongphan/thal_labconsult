...
SCREENING_COLUMNS = ["sample_id","age","sex","hb_g_dl","rbc_10e12_l","hct_percent","mcv_fl","mch_pg","mchc_g_dl","rdw_percent","retic_percent","ferritin_ng_ml","hba_percent","hba2e_percent","hbf_percent","hbe_percent","oft","dcip","hbh_inclusion","transfusion_recent","pregnant","smear_target_cells","family_history"]
...
def analyze_screening(row: dict[str, Any] | pd.Series, thresholds: dict[str, float] | None = None) -> ScreeningResult:
    ...
    if not np.isnan(final_hba2e):
        if final_hba2e > 25:
            top_pattern = "Suspected Hb E (Trait or Homozygous)"
            risk = "High"; consult_score = 100.0
        elif 3.5 <= final_hba2e <= 10:
