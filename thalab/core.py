...
def mcv_threshold(age: float, default_mcv: float = 80.0) -> float:
    if np.isnan(age) or age >= 15: return default_mcv
    if age < 1: return 70.0  
    if age < 2: return 72.0  
    if age < 3: return 74.0  
    if age <= 4: return 75.0 
    if age <= 7: return 76.0 
    if age <= 11: return 77.0
    return 78.0
...
def analyze_screening(row: dict[str, Any] | pd.Series, thresholds: dict[str, float] | None = None) -> ScreeningResult:
    ...
    # Robust real-world import layer: common analyzer/LIS exports use either
    # HbA2, HbA2/E, or HbA2E naming. Keep both fields available so the
    # scoring engine and the report layer remain consistent.
    ...
    for k in ["age","hb_g_dl","rbc_10e12_l","hct_percent","mcv_fl","mch_pg","mchc_g_dl","rdw_percent","retic_percent","ferritin_ng_ml","hba_percent","hba2_percent","hbf_percent","hbe_percent","hba2e_percent"]: n[k] = safe_num(n.get(k))
    ...
    dynamic_mcv_t = mcv_threshold(age, t["mcv_microcytosis"])
    
    anemia = not np.isnan(hb) and hb < dynamic_anemia_hb
    ...
    evidence=[]
    if anemia: evidence.append(f"Hb below age/sex/pregnancy-adjusted anemia threshold ({hb:g} g/dL < {dynamic_anemia_hb:g} g/dL).")
    ...
    # ---------------------------------------------------------
    # การนำเกณฑ์ Hb Typing ของคุณมาใช้ตัดสินและครอบทับ (Override)
    # ---------------------------------------------------------
