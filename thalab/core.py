...
SCREENING_COLUMNS = [
    "sample_id","age","sex","hb_g_dl","rbc_10e12_l","hct_percent","mcv_fl","mch_pg",
    "mchc_g_dl","rdw_percent","retic_percent","ferritin_ng_ml","hba_percent","hba2_percent",
    "hba2e_percent","hbf_percent","hbe_percent", "bart_percent", "hbh_percent", "hbcs_percent", "hb_method",
    "oft","dcip","hbh_inclusion","transfusion_recent","pregnant","smear_target_cells","family_history"
]

# ปรับ rdw_high เป็น 14.5 ตามตาราง
DEFAULT_THRESHOLDS = {"mcv_microcytosis":80.0,"mch_hypochromia":26.0,"hba2_beta_trait":3.5,"hbf_elevated":1.0,"hbe_present":10.0,"ferritin_deficient":15.0,"ferritin_borderline":30.0,"rbc_high":5.0,"mentzer_beta_like":13.0,"rdw_high":14.5,"severe_hb":7.0}
...
# ----- ชุดฟังก์ชันเกณฑ์ตามตารางใหม่ -----
def anemia_threshold(age: float, sex: str | None = None, pregnant: bool = False) -> float:
    if pregnant: return 11.0
    if np.isnan(age) or age >= 15:
...
def hct_threshold(age: float, sex: str | None = None, pregnant: bool = False) -> float:
    if pregnant: return 33.0
    if np.isnan(age) or age >= 15:
        s = (sex or "").strip().lower()
        return 40.0 if s.startswith("m") else 36.0
    if age < 1.0: return 42.0  # infant 
    if age < 10.0: return 33.0 # 1-9y
    if age < 13.0: return 35.0 # 10-12y
    return 36.0

def mcv_threshold(age: float) -> float:
    if np.isnan(age) or age >= 12: return 80.0
    if age < 14/365: return 98.0
    if age < 1/12: return 88.0
    if age < 2/12: return 91.0
    if age < 4/12: return 84.0
    if age < 6/12: return 76.0
    if age < 9/12: return 68.0
    if age < 1.0: return 70.0
    if age < 2.0: return 71.0
    if age < 6.0: return 75.0
    if age < 12.0: return 77.0
    return 80.0

def mch_threshold(age: float) -> float:
    if np.isnan(age) or age >= 12: return 26.0
    if age < 14/365: return 31.0
    if age < 1/12: return 30.0
    if age < 2/12: return 29.0
    if age < 4/12: return 27.0
    if age < 6/12: return 25.0
    if age < 9/12: return 24.0
    if age < 1.0: return 25.0
    if age < 6.0: return 24.0
    if age < 12.0: return 25.0
    return 26.0
# -----------------------------------
...
def analyze_screening(row: dict[str, Any] | pd.Series, thresholds: dict[str, float] | None = None) -> ScreeningResult:
    ...
    n = {c: source.get(c, np.nan) for c in SCREENING_COLUMNS}

    hba2_aliases = ["hba2_percent", "hba2e_percent", "hba2", "hba2e", "HbA2", "HbA2/E"]
    hba2_values = [source.get(k, np.nan) for k in hba2_aliases if k in source]
    first_hba2 = next((safe_num(v) for v in hba2_values if not np.isnan(safe_num(v))), np.nan)
    if np.isnan(safe_num(n.get("hba2_percent"))) and not np.isnan(first_hba2):
        n["hba2_percent"] = first_hba2
    ...
    n["hb_method"] = str(n.get("hb_method")) if not is_missing(n.get("hb_method")) else "HPLC"
    ...
    for k in ["age","hb_g_dl","rbc_10e12_l","hct_percent","mcv_fl","mch_pg","mchc_g_dl",
              "rdw_percent","retic_percent","ferritin_ng_ml","hba_percent","hba2_percent",
              "hbf_percent","hbe_percent","hba2e_percent", "bart_percent", "hbh_percent", "hbcs_percent"]: 
        n[k] = safe_num(n.get(k))
    
    d = derived_indices(n)
    hb, rbc, mcv, mch, rdw, ferritin, hct = n["hb_g_dl"], n["rbc_10e12_l"], n["mcv_fl"], n["mch_pg"], n["rdw_percent"], n["ferritin_ng_ml"], n["hct_percent"]
    hba2, hbf, hbe = n["hba2_percent"], n["hbf_percent"], n["hbe_percent"]
    bart, hbh, hbcs = n["bart_percent"], n["hbh_percent"], n["hbcs_percent"]
    age, sex, pregnant = n["age"], n["sex"], n["pregnant"]

    # --- ใช้เกณฑ์ใหม่ล่าสุด ---
    dynamic_anemia_hb = anemia_threshold(age, sex, pregnant)
    dynamic_anemia_hct = hct_threshold(age, sex, pregnant)
    dynamic_mcv_t = mcv_threshold(age)
    dynamic_mch_t = mch_threshold(age)
    
    # Anemia จะเช็คทั้ง Hb และ Hct (ตัวใดตัวหนึ่งต่ำ ถือว่าซีด)
    hb_anemia = not np.isnan(hb) and hb < dynamic_anemia_hb
    hct_anemia = not np.isnan(hct) and hct < dynamic_anemia_hct
    anemia = hb_anemia or hct_anemia
    
    micro = not np.isnan(mcv) and mcv < dynamic_mcv_t
    hypo = not np.isnan(mch) and mch < dynamic_mch_t
    # -------------------------
    ...
    evidence=[]
    if hb_anemia: evidence.append(f"Hb below age/sex/pregnancy-adjusted anemia threshold ({hb:g} g/dL < {dynamic_anemia_hb:g} g/dL).")
    if micro: evidence.append(f"Microcytosis: MCV {mcv:g} fL < {dynamic_mcv_t:g} fL (Adjusted for age).")
    if hypo: evidence.append(f"Hypochromia: MCH {mch:g} pg < {dynamic_mch_t:g} pg (Adjusted for age).")
    ...
    extra_hb = []
    if not np.isnan(bart) and bart > 0: extra_hb.append(f"Bart: {bart}%")
    if not np.isnan(hbh) and hbh > 0: extra_hb.append(f"HbH: {hbh}%")
    if not np.isnan(hbcs) and hbcs > 0: extra_hb.append(f"HbCS: {hbcs}%")
    
    if extra_hb:
        evidence.append(f"Abnormal Hb variants detected: {', '.join(extra_hb)} (Suggestive of α-thalassemia spectrum).")
        top_pattern = "Suspected α-thalassemia / HbH Disease Spectrum"
        risk = "High"
        consult_score = max(consult_score, 100.0)

    if not np.isnan(final_hba2e) and not extra_hb:
    ...
    if alpha >= 45 or hbh_pos or extra_hb: recommendations.append("Reflex to HBA1/HBA2 deletion-duplication analysis; add nondeletional α-globin variant testing when HbH/Constant Spring is suspected.")
...
def example_screening_dataframe() -> pd.DataFrame:
    ...
    df["hba2e_percent"] = df["hba2_percent"]
    # 5. เพิ่ม Mock data สำหรับสร้าง Template ไฟล์ CSV
    df["bart_percent"] = np.nan
    df["hbh_percent"] = np.nan
    df["hbcs_percent"] = np.nan
    df["hb_method"] = "HPLC"
    return df[[c for c in SCREENING_COLUMNS if c in df.columns]]